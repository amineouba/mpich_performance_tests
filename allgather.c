...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < NUM_OF_MSGS; i++) {
        ...
        for (int j = 0; j < NUM_TESTS; j++) {   
            send_data = (int*)malloc(message_size * sizeof(int));
            recv_data = (int*)malloc(size * message_size * sizeof(int));

            MPI_Barrier(MPI_COMM_WORLD);
            start_t = clock();
            MPI_Allgather(send_data, message_size, MPI_INT, recv_data, message_size, MPI_INT, MPI_COMM_WORLD);
            MPI_Barrier(MPI_COMM_WORLD);
            end_t = clock();
